package com.akdogan.simpleclickcounter.extension
...
private fun convertTimeStampToDateString(time: Long, pattern: String, tu: TimeUnit): String{
    val targetTime = TimeUnit.SECONDS.convert(time, tu)
    val date = Date(time)
...
/**
 * Returns a date string representation of the provided long
 */
...
/**
 * Return true if the provided Long is a point of time in the current day, taking into account Timezones
 */
fun Long.isToday(): Boolean {
    val zdtStart = nowZoned().toLocalDate().atStartOfDay(getSystemZone()).toEpochSecond()
    Log.d("DateHelper", "startOfDay: $zdtStart, timeInQuestion: $this")
    return this >= zdtStart
}

/**
 * Provides a long representing the start of day of today
 */
